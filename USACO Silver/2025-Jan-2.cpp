// Intended solution - median as optimal x + prefix sums
...
int main() {
    ...
    For (_i, _t) {
        ...
        ll median = N / 2;
        ll min_remainder = LLONG_MAX;

        For (i, N + 1) {    
            ll curr_remainder = abs(prefix[i + median] - prefix[i] - (median) * nums[i + median]) + (prefix[i + N] - prefix[i + median] - (N - median) * nums[i + median]);
            min_remainder = min(curr_remainder, min_remainder);
        }

        cout << min_remainder << nl;
    }
}

// Unintended solution - piecewise linearity + sweep line
